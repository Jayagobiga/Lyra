...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SERVER["CONTENT_TYPE"]) && strpos($_SERVER["CONTENT_TYPE"], "application/json") !== false) {
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        ...
        if ($userId) {
            ...
            // Query to fetch the fertileStartDate and fertileEndDate for the given userId
            $sql = "SELECT fertileStartDate, fertileEndDate FROM cycleupdate2 WHERE userid='$userId'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                // Fetch the dates
                $row = $result->fetch_assoc();
                $fertileStartDate = $row['fertileStartDate'];
                $fertileEndDate = $row['fertileEndDate'];
                echo json_encode([
